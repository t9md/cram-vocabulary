...
Config = {
  ...
  quizChoiceTextFilter: {
    definition: content => {
      // 以下の２つの事をやっている。
      // 1. 半角、または全角カッコの中に半角文字が含まれてたらカッコごとまるっと削除。
      // scripture という単語の選択肢で、"【名】（通常 the Scripture で）聖書" と選択肢に出てしまうと
      // 答えが分かってしまうのでまるごと消して、選択肢が "【名】聖書" となるようにしている。
      // 2. "<br>" または "、" で分割し、最初のフィールドをピックアップ。
      // 長いとヒントが豊富になって問題が簡単になってしまうし、読む文字が多いとサクサクこなせないので。
      // 例: "【副】反対に、逆に、対照的に、逆に言えば" を "【副】反対に" にして選択肢に出す。
      const regex = new RegExp('[（(][^)）]*?\\w+[^)）]*?[)）]', 'g')
      return content.replace(regex, '').split(/<br>|、/)[0]
    }
  }
}
